...
EOT;
		...
		&$table_comment = '')
	{
		...
		$class_name = ucfirst($classname_prefix) . $class_name_base;
		$table_cols = $this->rawQuery("SHOW FULL COLUMNS FROM " . Db_Query_Mysql::quotedIfBare($table_name))->execute()->fetchAll(PDO::FETCH_ASSOC);
		...
		$index_rows = $this->rawQuery("SHOW INDEX FROM " . Db_Query_Mysql::quotedIfBare($table_name))
			->execute()
